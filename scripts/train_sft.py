...
from src.training.sft_trainer import SFTTrainingConfig, train_sft
...
def create_config_from_yaml(yaml_config: dict, args) -> SFTTrainingConfig:
    """Create SFTTrainingConfig from YAML config and CLI args."""
    ...
    if args.debug or debug_cfg.get("enabled", False):
        max_samples = args.max_samples or debug_cfg.get("max_samples", 100)
    ...
    return SFTTrainingConfig(
        ...
        datasets_to_load=data_cfg.get("datasets", ["rlhfv", "pixmo", "spatial"]),
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum samples per dataset (debug)",
    )
    parser.add_argument(
        "--no_wandb",
        action="store_true",
    ...
    if args.debug and args.mlx:
        from src.training.mlx_debug import debug_sft_with_mlx

        yaml_config = load_config(args.config)
        data_cfg = yaml_config.get("data", {})
        max_samples = args.max_samples or 5

        results = debug_sft_with_mlx(
            model_name=args.mlx_model,
            datasets_to_load=data_cfg.get("datasets", ["rlhfv"]),
            max_samples=max_samples,
        )
    ...
    print(f"Datasets: {config.datasets_to_load}")
